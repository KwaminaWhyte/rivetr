...
- **Duplicate webhook deployments on push** — GitHub (and GitHub Apps) can deliver the same webhook event twice (two App installations, two webhook registrations, or a retry). Every delivery now carries the `X-GitHub-Delivery` header. Rivetr stores this ID in a new `delivery_id` column (migration 092, unique index) and rejects any second delivery for the same ID before processing, preventing double deploys.
