...
- **Database overview showed fake resource-limit placeholders** — when a database had no CPU/memory limit set, the overview rendered hardcoded `512mb` / `0.5` defaults, implying a cap that wasn't actually applied (e.g. a DB using 2.4 GB displaying a "512mb" limit). It now shows the configured value or "No limit".
- **Live memory-limit update failed on containers started without a cap** — `update_container_limits` (apps' apply-limits and the new DB path) set `memory` without `memory_swap`, which Docker rejects with "Memory limit should be smaller than already set memoryswap limit" whenever the container had unlimited swap. It now sets `memory_swap == memory` so the update is accepted and the cap is hard.
