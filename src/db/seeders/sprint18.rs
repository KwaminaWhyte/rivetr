...
"#,
        ...
        (
            "tpl-activepieces",
            "Activepieces",
            "Open-source business automation platform. Build workflows with a no-code builder and 100+ integrations including AI tools.",
            "automation",
            "activepieces",
            r#"services:
  activepieces:
    ...
    environment:
      ...
      - AP_ENCRYPTION_KEY=${ENCRYPTION_KEY:-changethiskeynow16}
      - AP_JWT_SECRET=${JWT_SECRET:-changethisjwtsecret}
      - AP_ENVIRONMENT=prod
      ...
      - AP_POSTGRES_PASSWORD=${POSTGRES_PASSWORD:-changeme}
  ...
  postgres:
    image: postgres:16-alpine
    container_name: ${CONTAINER_NAME:-activepieces}-postgres
    restart: unless-stopped
    environment:
...
"#,
            r#"[{"name":"ENCRYPTION_KEY","label":"Encryption Key (16+ chars)","required":true,"default":"changethiskeynow16","secret":true},{"name":"JWT_SECRET","label":"JWT Secret","required":true,"default":"changethisjwtsecret","secret":true},{"name":"POSTGRES_PASSWORD","label":"PostgreSQL Password","required":true,"default":"changeme","secret":true},{"name":"FRONTEND_URL","label":"Frontend URL","required":false,"default":"http://localhost:8080","secret":false},{"name":"PORT","label":"Host Port","required":false,"default":"8080","secret":false}]"#,
        ),
...
"#,
        ...
        (
            "tpl-ntfy-server",
            "ntfy",
            "Simple HTTP-based pub-sub notification service. Send push notifications to your phone or desktop using HTTP PUT or POST.",
            "communication",
            "ntfy",
            r#"services:
  ntfy:
    ...
    command: serve
    environment:
      - NTFY_BASE_URL=${BASE_URL:-http://localhost:8090}
      - NTFY_BEHIND_PROXY=${BEHIND_PROXY:-false}
    volumes:
...
volumes:
  ...
  ntfy_etc:
"#,
            r#"[{"name":"BASE_URL","label":"Base URL","required":false,"default":"http://localhost:8090","secret":false},{"name":"PORT","label":"Host Port","required":false,"default":"8090","secret":false},{"name":"BEHIND_PROXY","label":"Behind Proxy","required":false,"default":"false","secret":false}]"#,
        ),
        (
...
"#,
        ...
        (
            "tpl-filebrowser",
            "Filebrowser",
            "Web-based file browser with sharing, editing, and user management. Clean UI for managing files on any server.",
            "storage",
            "filebrowser",
            r#"services:
  filebrowser:
...
volumes:
  ...
  filebrowser_db:
"#,
            r#"[{"name":"PORT","label":"Host Port","required":false,"default":"8083","secret":false},{"name":"BASE_URL","label":"Base URL","required":false,"default":"/","secret":false}]"#,
        ),
        (
            "tpl-storj-gateway",
            "Storj Gateway MT",
...
"#,
        ...
        (
            "tpl-vaultwarden",
            "Vaultwarden",
            "Lightweight Bitwarden-compatible server. Self-host your password manager with full Bitwarden client support.",
            "security",
            "vaultwarden",
            r#"services:
  vaultwarden:
...
volumes:
  vaultwarden_data:
"#,
            r#"[{"name":"ADMIN_TOKEN","label":"Admin Token","required":true,"default":"changeme","secret":true},{"name":"SIGNUPS_ALLOWED","label":"Allow Signups","required":false,"default":"true","secret":false},{"name":"PORT","label":"Host Port","required":false,"default":"8084","secret":false}]"#,
        ),
        (
            "tpl-infisical-server",
            "Infisical",
            "Open-source secrets management platform. Sync environment variables and secrets across your team and infrastructure.",
            "security",
            "infisical",
            r#"services:
  infisical:
    ...
    environment:
      ...
      - AUTH_SECRET=${AUTH_SECRET:-changeauthsecret32charslongatleast}
  ...
  postgres:
    image: postgres:16-alpine
    container_name: ${CONTAINER_NAME:-infisical}-postgres
    restart: unless-stopped
    environment:
  ...
  redis:
    image: redis:7-alpine
    container_name: ${CONTAINER_NAME:-infisical}-redis
    restart: unless-stopped
    volumes:
...
"#,
            r#"[{"name":"ENCRYPTION_KEY","label":"Encryption Key (32 chars)","required":true,"default":"6c1fe4e407b8911c104518103505b218","secret":true},{"name":"AUTH_SECRET","label":"Auth Secret (32+ chars)","required":true,"default":"changeauthsecret32charslongatleast","secret":true},{"name":"POSTGRES_PASSWORD","label":"PostgreSQL Password","required":true,"default":"infisical","secret":true},{"name":"SITE_URL","label":"Site URL","required":false,"default":"http://localhost:8085","secret":false},{"name":"PORT","label":"Host Port","required":false,"default":"8085","secret":false}]"#,
        ),
        (
            "tpl-keycloak-server",
            "Keycloak",
            "Enterprise-grade identity and access management. SSO, OAuth 2.0, OIDC, SAML, and user federation in one platform.",
            "security",
            "keycloak",
            r#"services:
  keycloak:
    ...
    environment:
      ...
      - KEYCLOAK_ADMIN_PASSWORD=${ADMIN_PASSWORD:-changeme}
      - KC_DB=postgres
  ...
  postgres:
    image: postgres:16-alpine
    container_name: ${CONTAINER_NAME:-keycloak}-postgres
    restart: unless-stopped
    environment:
...
volumes:
  keycloak_pg_data:
"#,
            r#"[{"name":"ADMIN_USER","label":"Admin Username","required":false,"default":"admin","secret":false},{"name":"ADMIN_PASSWORD","label":"Admin Password","required":true,"default":"changeme","secret":true},{"name":"POSTGRES_PASSWORD","label":"PostgreSQL Password","required":true,"default":"keycloak","secret":true},{"name":"KC_HOSTNAME","label":"Hostname","required":false,"default":"localhost","secret":false},{"name":"PORT","label":"Host Port","required":false,"default":"8086","secret":false}]"#,
        ),
...
"#,
        ...
        (
            "tpl-focalboard-server",
            "Focalboard",
            "Open-source project management tool. Kanban boards, tables, and galleries to organize your team's work.",
            "productivity",
            "focalboard",
            r#"services:
  focalboard:
...
"#,
        ...
        (
            "tpl-strapi-postgres",
            "Strapi (PostgreSQL)",
            "Open-source headless CMS with PostgreSQL backend. Build APIs fast with a customizable admin panel and content types.",
            "cms",
            "strapi",
            r#"services:
  strapi:
  ...
  postgres:
    image: postgres:16-alpine
    container_name: ${CONTAINER_NAME:-strapi}-postgres
    restart: unless-stopped
    environment:
...
volumes:
  ...
  strapi_pg_data:
"#,
            r#"[{"name":"APP_KEYS","label":"App Keys (comma-separated)","required":true,"default":"key1,key2","secret":true},{"name":"ADMIN_JWT_SECRET","label":"Admin JWT Secret","required":true,"default":"changeme","secret":true},{"name":"JWT_SECRET","label":"JWT Secret","required":true,"default":"changeme","secret":true},{"name":"POSTGRES_PASSWORD","label":"PostgreSQL Password","required":true,"default":"strapi","secret":true},{"name":"PORT","label":"Host Port","required":false,"default":"1337","secret":false}]"#,
        ),
        (
            "tpl-ghost",
            "Ghost",
            "Professional blogging and newsletter platform. Modern publishing tools, membership features, and built-in SEO.",
            "cms",
            "ghost",
            r#"services:
  ghost:
    ...
    environment:
      ...
      - database__connection__database=${MYSQL_DB:-ghost}
  ...
  mysql:
    image: mysql:8
    container_name: ${CONTAINER_NAME:-ghost}-mysql
    restart: unless-stopped
    environment:
      - MYSQL_ROOT_PASSWORD=${MYSQL_ROOT_PASSWORD:-rootpassword}
      - MYSQL_DATABASE=${MYSQL_DB:-ghost}
...
volumes:
  ...
  ghost_mysql_data:
"#,
            r#"[{"name":"SITE_URL","label":"Site URL","required":false,"default":"http://localhost:2368","secret":false},{"name":"MYSQL_PASSWORD","label":"MySQL Password","required":true,"default":"changeme","secret":true},{"name":"MYSQL_ROOT_PASSWORD","label":"MySQL Root Password","required":true,"default":"rootpassword","secret":true},{"name":"PORT","label":"Host Port","required":false,"default":"2368","secret":false}]"#,
        ),
        (
            "tpl-write-freely",
            "Write.freely",
...
"#,
        ...
        (
            "tpl-cal-com-server",
            "Cal.com",
            "Open-source Calendly alternative. Scheduling infrastructure for individuals, teams, and enterprises with full customization.",
            "productivity",
            "cal-com",
            r#"services:
  calcom:
    ...
    ports:
      - "${PORT:-3002}:3000"
    environment:
      - NEXT_PUBLIC_WEBAPP_URL=${WEBAPP_URL:-http://localhost:3002}
      - NEXTAUTH_URL=${WEBAPP_URL:-http://localhost:3002}
      - NEXTAUTH_SECRET=${NEXTAUTH_SECRET:-changethisnextauthsecret}
      - DATABASE_URL=postgresql://${POSTGRES_USER:-calcom}:${POSTGRES_PASSWORD:-calcom}@postgres:5432/${POSTGRES_DB:-calcom}
      - CALENDSO_ENCRYPTION_KEY=${ENCRYPTION_KEY:-changethisencryptionkey}
    depends_on:
      - postgres
    labels:
      - "rivetr.managed=true"
  ...
  postgres:
    image: postgres:16-alpine
    container_name: ${CONTAINER_NAME:-calcom}-postgres
    restart: unless-stopped
    environment:
      - POSTGRES_DB=${POSTGRES_DB:-calcom}
      - POSTGRES_USER=${POSTGRES_USER:-calcom}
      - POSTGRES_PASSWORD=${POSTGRES_PASSWORD:-calcom}
    volumes:
...
volumes:
  calcom_pg_data:
"#,
            r#"[{"name":"WEBAPP_URL","label":"Web App URL","required":false,"default":"http://localhost:3002","secret":false},{"name":"NEXTAUTH_SECRET","label":"NextAuth Secret","required":true,"default":"changethisnextauthsecret","secret":true},{"name":"ENCRYPTION_KEY","label":"Encryption Key","required":true,"default":"changethisencryptionkey","secret":true},{"name":"POSTGRES_PASSWORD","label":"PostgreSQL Password","required":true,"default":"calcom","secret":true},{"name":"PORT","label":"Host Port","required":false,"default":"3002","secret":false}]"#,
        ),
        (
            "tpl-dolibarr-erp",
            "Dolibarr ERP/CRM",
