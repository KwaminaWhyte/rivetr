...
pub async fn create_build_server(
    ...
    sqlx::query(
        r#"
        INSERT INTO build_servers (id, name, host, port, username, ssh_private_key, ssh_password, status, concurrent_builds, team_id, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, 'unknown', ?, ?, ?, ?)
        "#,
    )
    .bind(&id)
...
pub async fn check_build_server_health(
    ...
    match health_result {
        ...
        Err(e) => {
            ...
            // Mark as offline and update last_seen_at (check was attempted)
            sqlx::query(
                "UPDATE build_servers SET status = 'offline', last_seen_at = ?, updated_at = ? WHERE id = ?"
            )
            .bind(&now)
            .bind(&now)
            .bind(&id)
