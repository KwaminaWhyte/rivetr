//! Sendry email API notification sender.
//!
//! Sends transactional emails via the Sendry API (https://sendry.online).
//! Sendry exposes a Resend-compatible HTTP API, so this sender mirrors the
//! Resend sender but targets Sendry's endpoint.

use anyhow::Result;
use serde_json::json;

use crate::db::SendryConfig;

use super::NotificationPayload;

/// Sendry API endpoint for sending emails
const SENDRY_API_URL: &str = "https://api.sendry.online/v1/emails";

/// Send an email notification via the Sendry API
pub async fn send_sendry(
