...
>
> **Pre-existing bug — FIXED 2026-06-10 (commit 2608a6d):** startup logs showed repeated `WARN container_monitor::health: Failed to fetch running services error=no column found for name: cpu_limit`. Root cause was NOT a missing migration (columns existed) — the health monitor's explicit-column `SELECT ... FROM services` omitted `cpu_limit`/`memory_limit` (added in migration 108) while decoding into the `Service` struct, so the `FromRow` decode failed and running compose services were never health-checked. Fixed `src/engine/container_monitor/health.rs` to select both columns; redeployed; 0 errors since restart.
