...
- [x] **FIXED — fail-closed** (`webhooks/github.rs`, `gitlab.rs`, `gitea.rs`, `bitbucket.rs`): a secret MUST be configured and the signature/token MUST verify, else 401. Previously verification was skipped when the secret was unset.
  - **GitHub App support:** the handler verifies the signature against *any* candidate secret — the global `webhooks.github_secret` **and** every connected GitHub App's own `webhook_secret` (decrypted from `github_apps`). GitHub Apps sign with their per-App secret, so this is required for App-based installs and means **no global config and no re-linking are needed**.
  - **Operational (server `187.124.50.183`):** integration is 3 GitHub Apps (plaintext App secrets, no encryption_key). After the App-secret fix, live test: no-sig → 401, wrong-secret → 401, each of the 3 App secrets → 200. CD restored with **zero GitHub-side changes**. A global `webhooks.github_secret` was also provisioned but is not required (harmless extra candidate).
  - Still open (enhancement): per-app/per-repo secrets in DB for non-App webhooks.
