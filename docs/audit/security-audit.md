...
- [ ] **Fix:** Move `/mcp` behind `auth::auth_middleware` (nest under protected `/api` group). Require valid admin/API token before dispatching any tool.
...
- [ ] **Fix:** Add `is_pending_2fa` (or `scope`) column to `sessions`; set for temp session, reject pending in `auth_middleware`/`validate_ws_token`, clear in `validate_2fa`. Or store pre-2FA challenge in a separate table the auth middleware never consults.
...
- [ ] **Fix:** Add an authorization layer that resolves each resource's `team_id` and calls `require_team_role` for the authenticated user on every app/server/database/env-var/file/terminal/volume handler. Never trust client-supplied `team_id` for list scoping — derive the user's teams server-side.
...
- [ ] **Fix:** Stop building shell strings — pass path as separate argv element to non-shell remote invocation, or reject paths with shell metacharacters (`$`, backtick, `;`, `|`, `&`, `(`, `)`, newline) and use sftp/SCP. Enforce server ownership first.
